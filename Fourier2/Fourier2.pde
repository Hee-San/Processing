...
void setup() {
  ...
  for(int i = 0; i < 4; i++){
    cl1.add(new circle(width/12, i*height/4+height/8, 0, 150/(i+2), (i+2)));
  }
...
void draw() {
  ...
  for(int i = 1; i <= NCircle; i++){
    NewY -= 150/i*sin(frameCount*PI/100*i);
  }
  wave();
  c.draw();
...
void mouseClicked(){
  ...
  cl1.add(new circle(width/12, 3*height/4+height/8, 0, 150/(NCircle+5), NCircle+5));
  NCircle ++;
  Fomula = Fomula + "+1/" + NCircle + "*sin(" + NCircle + "x)";
}